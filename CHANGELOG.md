...
### Planned- ⭐ **Expand DEALS_DB → 500+ deals** (top community-contribution priority — currently 328)
...
- Side-by-side scenario comparison- PDF / one-pager export · PDF /
- Save/load asset profiles via URL hash ·  URL hash
- Stage-conditional WACC defaults ·  WACC
...
#### Added- **DEALS_DB expanded 53 → 328 entries** via bulk ingest from public deal data covering 2024-11 → 2026-02
...
#### Changed- All "**380+ global buyers**" copy replaced with the real `BUYERS.length` (currently 75 deeply profiled buyers) — same approach used for deal count in v1.0.1
...
#### Notes- The bulk-ingest yield (280 from 1,196 raw rows) reflects strict filtering: only deals with disclosed total dollar amounts in // USD/EUR/GBP/JPY/HKD/RMB/KRW were accepted; FX-converted to USD
...
#### Added- **`compImpliedValue()`** — similarity-weighted regression on `DEALS_DB` with stage / modality / territory corrections
...
#### Changed- `findComps()` similarity gate tightened — requires real indication-or-modality match, not just stage adjacency
...
#### Fixed- Marketing copy throughout the UI no longer references the inflated "1,196" figure — replaced with live database count (currently 53 hand-curated cross-border deals)
...
#### Added- 🎯 Buyer matching across 380+ global strategic acquirers (portfolio fit, geography, recent deals)
...
- 🌐 Bilingual UI ( / English) with stage-conditional parameter display
...
#### Open-sourced under- MIT License — see [LICENSE](LICENSE)
